...
function watershed(img::AbstractArray{T, N},
    ...
    for i in R
        if markers[i] > 0
            for j in CartesianIndices(_colon(max(Istart,i-oneunit(i)), min(i+oneunit(i),Iend)))
                if segments[j] == 0
    ...
    while !isempty(pq)
        ...
        for j in CartesianIndices(_colon(max(Istart,curr_idx-oneunit(curr_idx)), min(curr_idx+oneunit(curr_idx),Iend)))
